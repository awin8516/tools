          ...
          <div class="user_info_data">
            <el-card class="box-card">
              ...
              <div class="remarks" v-for="(item,index) in showTag(diyTag)" :key="index">
                <p class="remark">
                  <span class="delete">
                    <i class @click="diy_biao_qian_view(item)">查看</i>
                    <i class @click="diy_biao_qian_remove(item)">删除</i>
                  </span>
                  {{item.label}}
                </p>
              </div>
...
export default {
  ...
  data() {
    return {
      danger_type: "用户自定义标签",
      danger_ontent: "删除用户自定义标签，此操作会将所有删除用户自定义标签",
      danger_item: "",
      backTips: "返回用户列表页",
  ...
  methods: {
    ...
    // 查看用户自定义标签的列表
    diy_biao_qian_view(item) {
      // window.open(
      //   `${this.GLOBAL.DOMAIN}/home.html#/UserInfo?tid=${window.btoa(
      //     item.value
      //   )}`,
      //   "_blank"
      // );
      localStorage.setItem('cid',window.btoa(item.value))
      // window.open(
