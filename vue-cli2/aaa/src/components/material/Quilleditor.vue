<template>
  <div>
    ...
    ></crop-upload>
    <!-- 备注 -->
    <el-dialog title="选择卡券" :visible.sync="dialogVisible" width="60%" :close-on-click-modal="modal">
      <v-coupon ref="coupon" @listenChild="showChild"></v-coupon>
    </el-dialog>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    showChild(data) {
      this.$axios
        .post(this.GLOBAL.BASE_URL + "createCouponCode", {
    ...
    onChange() {
      ...
      this.$nextTick(() => {
        ...
        //   let img = contentWindow.document.getElementById("js_logo_url");
    ...
    onUploadSuccess: function(res) {
      ...
      this.editor.insertEmbed(
        this.editor.getSelection().index,
        "121212",
        "https://scrm.beats-digital.com/Ajax/Media/ShowImage?image=" +
          res.data.url
      );
    },
    /*点击上传图片按钮*/
